...
import { Trophy, Users, Calendar, ListChecks, ChevronRight, Activity } from "lucide-react";
        ...
        <div className="min-h-screen">
            <div className="flex-1 p-4">
                ...
                <div className="mb-6">
                    ...
                    <p className="text-muted-foreground">Here's your team's progress overview</p>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-6">
                    ...
                    <StatCard
                        title="Team Progress"
                        value={`${Math.round((teamTasks.filter(t => t.status === "completed").length / teamTasks.length) * 100)}%`}
                                ...
                                {['pending', 'in_progress', 'completed'].map((status) => (
                                    <div key={status} className="flex justify-between items-center">
                                        ...
                                        <span className="text-sm font-medium">
                                            {assignedTasks.filter(t => t.status === status).length}
                                        </span>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="space-y-4">
                                {['pending', 'in_progress', 'completed'].map((status) => (
                                    <div key={status} className="flex justify-between items-center">
                                        ...
                                        <span className="text-sm font-medium">
                                            {teamTasks.filter(t => t.status === status).length}
                                        </span>
                                    </div>
                                ))}
                            </div>
