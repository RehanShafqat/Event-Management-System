...
const MFASetup = () => {
    ...
    const { user, qrCode, secret, mfaRequired, setupMfa } = useSelector((state) => state.authentication);
    ...
    const handleVerify = async (e) => {
        ...
        try {
            const resultAction = await dispatch(verifyMfa({
                userId: user.id,
                code: code
            }));
            ...
            if (verifyMfa.fulfilled.match(resultAction)) {
                ...
                toast.error(resultAction.payload?.message || "Failed to verify MFA code", { id: toastId });
            }
        } catch {
            ...
            <Card className="w-[400px] bg-[#1a1a1a] border border-[#2a2a2a]">
                <CardHeader>
                    <h1 className="text-2xl font-bold text-center text-white">
                        {mfaRequired ? "Verify Two-Factor Authentication" : "Setup Two-Factor Authentication"}
                    </h1>
                </CardHeader>
                <CardContent>
                    {error && (
                    ...
                    <div className="space-y-6">
                        ...
                        <form onSubmit={handleVerify} className="space-y-4">
                            ...
                            <Button
                                ...
                                disabled={loading}
                            >
                                {loading ? "Verifying..." : mfaRequired ? "Verify" : "Verify & Complete Setup"}
                            </Button>
                        </form>
                    </div>
                </CardContent>
