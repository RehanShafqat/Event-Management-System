...
import { updateMeeting } from "../../Redux/features/meetingSlice"
...
const EditMeeting = ({ meeting, onSuccess }) => {
    const dispatch = useDispatch()
    const { user } = useSelector((state) => state.authentication)

    // Extract time from meeting dateTime
    ...
    // Get the initial selected role from the meeting's attendees
    const getInitialSelectedRole = () => {
        if (!meeting.attendees || meeting.attendees.length === 0) {
            return []
        }
        // Get the lowest role from the attendees array
        const roleOrder = ["President", "VP", "AVP", "Head", "Deputy", "Officer"]
        const attendeeRoles = meeting.attendees.map((attendee) => attendee.role)

        const lowestRole = attendeeRoles.reduce((lowest, current) => {
            const currentIndex = roleOrder.indexOf(current)
            const lowestIndex = roleOrder.indexOf(lowest)
            return currentIndex > lowestIndex ? current : lowest
        })
        return [lowestRole]
    }

    const initialRoles = getInitialSelectedRole()
    ...
    const onSubmit = async (values) => {
        try {
            ...
            await dispatch(updateMeeting({ id: meeting._id, meetingData })).unwrap()
            onSuccess()
        } catch (error) {
